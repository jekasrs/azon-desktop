...
public class ManagerPurchaseReport implements Controller {
    @FXML
    BorderPane ap;
    ...
    public void fillTable(List<User> managers) {
        ...
        TableColumn amountCol = new TableColumn("Кол-во закупок");
        TableColumn sumCol    = new TableColumn("Сумма закупок");
    ...
    public void handleAnalyze(ActionEvent actionEvent) {
        ...
        try {
            if (filter.getText() == null || filter.getText().equals("")) {
                List<User> managersAnalyse = userRepository.getAnalyzeOfPurchaseManager(dateBeg, dateEnd);
                ...
                if (!verifyFio(filterValue))
                    return;
                User analyzeOfOneGoods = userRepository.getAnalyzeOfPurchaseManager(dateBeg, dateEnd, filterValue);
    ...
    private boolean verifyFio(String fio) throws SQLException {
        List<String> fioes = userRepository.getFIOofPurchaseManagers();
